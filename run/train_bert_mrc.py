...
from run.generate_train_dev import generate_flat_ner_dta

# parser.add_argument("--config_path", default="config/en_bert_base_uncased.json", type=str)
#     parser.add_argument("--data_dir", default='/data/iceburg/KG_Covid19_Task1/KG_Covid19_Task1/data', type=str)
#     parser.add_argument("--bert_model", default="/model/iceburg/bert-large-uncased", type=str,)
...
def load_data(config):
    ...
    # return train_dataloader, num_train_steps, label_list
...
def train(config):
    ...
    for idx in range(int(config.num_train_epochs)):
        # 每一轮训练重新加载一次数据
        generate_flat_ner_dta()
        train_dataloader, dev_dataloader, num_train_steps, label_list = load_data(config)
...
def eval_checkpoint(model_object, eval_dataloader, config, \
    ...
    if config.entity_sign == "flat":
        eval_accuracy, eval_precision, eval_recall, eval_f1, pred_bmes_label_lst = flat_ner_performance(start_pred_lst, end_pred_lst, span_pred_lst, start_gold_lst, end_gold_lst, span_gold_lst, ner_cate_lst, label_list, threshold=config.entity_threshold, dims=2)
    else:
        eval_accuracy, eval_precision, eval_recall, eval_f1, pred_bmes_label_lst = nested_ner_performance(start_pred_lst, end_pred_lst, span_pred_lst, start_gold_lst, end_gold_lst, span_gold_lst, ner_cate_lst, label_list, threshold=config.entity_threshold, dims=2)
