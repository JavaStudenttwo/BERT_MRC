...
def generate_query_ner_dataset(train, tokenizer, source_file_path, dump_file_path, entity_sign="nested",
    ...
    target_data = transform_examples_to_qa_features(tokenizer, entity_queries, label_lst, source_data, entity_sign=entity_sign)

    with open(dump_file_path, "w", encoding='utf-8') as f:
        json.dump(target_data, f, sort_keys=True, ensure_ascii=False, indent=2)


def transform_examples_to_qa_features(tokenizer, query_map, entity_labels, data_instances, entity_sign="nested"):
    """
    Desc:
    ...
    """ 
    mrc_ner_dataset = []
    bert_dict = tokenizer.vocab
    # if entity_sign.lower() == "flat":
    #     tmp_qas_id = 0
    #     for idx, sent_detial in enumerate(data_instances):
    #         text = sent_detial['text']
    #         text_list = list(text)
    #         text = []
    #         for i in text_list:
    #             if i in bert_dict:
    ...
    #                 text.append("[UNK]")
    #
    #         span_list = sent_detial['span_list']
    #         tmp_query_id = 0
    #         for label_idx, tmp_label in enumerate(entity_labels):
    #             tmp_query_id += 1
    #             tmp_query = query_map[tmp_label]
    #             tmp_context = " ".join(list(text))
    #
    #             tmp_start_pos = []
    #             tmp_end_pos = []
    #             tmp_entity_pos = []
    #
    #             start_end_label = [(start, end) for [start, end, label_content] in span_list if label_content == tmp_label]
    #
    #             if len(start_end_label) != 0:
    #                 for span_item in start_end_label:
    #                     start_idx, end_idx = span_item
    #                     tmp_start_pos.append(int(start_idx))
    #                     tmp_end_pos.append(int(end_idx))
